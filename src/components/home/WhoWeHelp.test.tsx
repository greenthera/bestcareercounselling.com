...
describe('WhoWeHelp', () => {
  it('renders all seven service cards linking to /what-we-do anchors', () => {
    render(
      <MemoryRouter>
        <WhoWeHelp />
      </MemoryRouter>,
  ...
  it('marks the focused service as the active one', async () => {
    ...
    expect(screen.getByRole('link', { name: /after 8th/i })).toHaveAttribute('aria-current', 'true')
    ...
    expect(screen.getByRole('link', { name: /after 8th/i })).not.toHaveAttribute('aria-current')
  })
})
